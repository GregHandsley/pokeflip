...
export function DraftCart({
  ...
  const load = useCallback(async () => {
    const { data, error } = await supabase
      .from("intake_lines")
      .select(
        "id, card_id, set_id, condition, variation, quantity, for_sale, list_price_pence, note, cards(number, name, api_image_url, api_payload), sets(name)"
      )
    ...
    if (newlyAddedSetId && newlyAddedCardId) {
      ...
      const cardLines = loadedLines.filter(
        (l) => l.set_id === newlyAddedSetId && l.card_id === newlyAddedCardId
      );
  ...
  const commit = async () => {
    ...
    setMsg(null);
    try {
      // @ts-expect-error - Supabase types don't properly infer RPC parameter types
      const { data, error } = await supabase.rpc("commit_acquisition", {
        p_acquisition_id: acquisitionId,
      });
      if (error) throw error;
      setMsg((data as CommitAcquisitionResponse)?.message ?? "Committed");
      await load();
      onCommitted?.();
    } catch (e: unknown) {
  ...
  return (
    <section className="rounded-2xl border border-black/10 bg-white p-6 flex flex-col h-full min-h-0">
      ...
      <div className="mt-6 flex items-center justify-between pt-4 border-t border-gray-200 shrink-0">
        <button
          type="button"
          onClick={commit}
