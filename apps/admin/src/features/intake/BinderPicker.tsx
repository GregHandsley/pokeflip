...
type CardRow = { id: string; set_id: string; number: string; name: string; api_image_url: string | null };
    ...
    <section className="rounded-2xl border border-black/10 bg-white p-4">
      <div className="flex items-start justify-between gap-4">
        <div className="w-full">
          ...
          <p className="mt-1 text-sm text-black/60">Choose a set, then click cards to add them to your draft cart.</p>
            ...
            >
              ...
              {sets.map(s => (
                <option key={s.id} value={s.id}>
                  {s.name}{s.release_date ? ` (${s.release_date})` : ""}
                </option>
              ))}
            </select>
          </label>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {[6,9,12].map(v => (
              <button
                key={v}
                onClick={() => { setLayout(v as BinderLayout); setPage(0); }}
      ...
      >
        {pageCards.map((c) => (
          ...
          >
            <div className="aspect-[2.5/3.5] w-full overflow-hidden rounded-lg border border-black/10 bg-black/5 flex items-center justify-center">
              {c.api_image_url ? (
                <img src={`${c.api_image_url}/low.webp`} alt="" className="h-full w-full object-cover" />
              ) : (
