...
export function SetAccordion({
  ...
  const cardGroups = useMemo(() => {
    return Array.from(groupLinesByCard(setLines).entries())
      .sort(([cardIdA, cardLinesA], [cardIdB, cardLinesB]) => {
        // Sort by card number (localId or number field)
        const firstLineA = cardLinesA[0];
        const firstLineB = cardLinesB[0];
        const localIdA = firstLineA.cards?.api_payload?.localId;
        const numberA = firstLineA.cards?.number?.split('-')[0]?.trim();
        const localIdB = firstLineB.cards?.api_payload?.localId;
      ...
      {isExpanded && (
        <div className="border-t border-black/10">
          {cardGroups.map(([cardId, cardLines]) => {
            ...
            const localId = firstLine.cards?.api_payload?.localId;
            ...
            const imageUrl = firstLine.cards?.api_image_url ? `${firstLine.cards.api_image_url}/high.webp` : null;
            // Auto-expand if this is the newly added card and it has multiple entries
            const shouldAutoExpand = newlyAddedCardId === cardId && (cardLines.length > 1 || cardLines.some(l => l.quantity > 1));
