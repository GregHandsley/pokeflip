...
export async function DELETE(req: Request, { params }: { params: Promise<{ cardId: string }> }) {
  ...
  try {
    ...
    // Delete lot photos
    const { error: photosError } = await supabase.from("lot_photos").delete().in("lot_id", lotIds);
    if (photosError) {
      logger.warn("Failed to delete lot photos", undefined, {
        cardId,
    ...
    const { error: ebayError } = await supabase.from("ebay_listings").delete().in("lot_id", lotIds);
    if (ebayError) {
      logger.warn("Failed to delete eBay listings", undefined, {
        cardId,
        lotIdsCount: lotIds.length,
        error: ebayError,
      });
    ...
    const { error: salesError } = await supabase.from("sales_items").delete().in("lot_id", lotIds);
    if (salesError) {
      logger.warn("Failed to delete sales items", undefined, {
        cardId,
        lotIdsCount: lotIds.length,
        error: salesError,
      });
    ...
    const { error } = await supabase.from("inventory_lots").delete().eq("card_id", cardId);

    if (error) {
      logger.error("Failed to delete inventory lots", error, undefined, {
        cardId,
        lotIdsCount: lotIds.length,
      });
      return createErrorResponse(
