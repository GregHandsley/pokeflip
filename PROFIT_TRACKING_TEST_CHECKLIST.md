...
### Verify Data Saves
- [ ] Create a sale with consumables
- [ ] Refresh the page
- [ ] Go to Sales & Profit
- [ ] Verify the sale appears
- [ ] Click "View Details"
- [ ] Verify all consumables are saved correctly
- [ ] Verify profit calculation matches what was shown in modal

### Verify Inventory Updates
- [ ] Mark a lot as sold (quantity 1, available 1)
- [ ] Verify lot status changes to "sold"
- [ ] Verify lot appears in "Sold Lots" collapsed section
- [ ] Mark another lot as sold (quantity 5, available 3, sell 2)
- [ ] Verify available quantity decreases to 1
- [ ] Verify status remains active (not sold)
...
### Full Workflow
1. [ ] Add consumables (sleeves, toploaders, etc.)
2. [ ] Record purchases for each consumable
3. [ ] Create packaging rules (single card, multi-card)
4. [ ] Mark a single card as sold
5. [ ] Verify consumables auto-apply
6. [ ] Verify profit calculation
7. [ ] Submit sale
8. [ ] Go to Sales & Profit reports
9. [ ] Verify sale appears with correct profit
10. [ ] View order details
11. [ ] Verify all data matches

### Repeat Buyer
- [ ] Mark item as sold to "buyer1"
- [ ] Mark another item as sold to "buyer1"
- [ ] When entering "buyer1" again, verify:
  - [ ] Autocomplete shows "buyer1"
  - [ ] Shows "Repeat buyer: 2 orders"
  - [ ] Shows total spend

### Order Grouping
- [ ] Mark multiple items as sold with same order group "ORDER-001"
- [ ] Verify order group appears in suggestions
...
### Navigation
- [ ] Verify "Sales & Profit" appears in main nav
- [ ] Verify "Settings" section appears in sidebar
- [ ] Verify Settings sub-items (Consumables, Packaging Rules) appear
...
### Visual Feedback
- [ ] Verify loading states show when fetching data
- [ ] Verify success/error messages appear appropriately
- [ ] Verify profit colors (green/red) are clear
- [ ] Verify warning messages are visible

---

## 11. Performance Testing

- [ ] Test with many consumables (50+)
- [ ] Test with many sales orders (100+)
- [ ] Verify profit calculation is fast (real-time updates)
- [ ] Verify reports page loads quickly with many orders
...
- [ ] Verify `consumables` table has entries
- [ ] Verify `consumable_purchases` table has entries
- [ ] Verify `packaging_rules` table has entries
- [ ] Verify `packaging_rule_items` table has entries
- [ ] Verify `sales_orders` table has entries with optional fields
- [ ] Verify `sales_consumables` table has entries
- [ ] Verify `v_consumable_costs` view returns correct averages
- [ ] Verify `v_sales_order_profit` view returns correct calculations
...
1. [ ] Add one consumable and record a purchase
2. [ ] Create a default packaging rule with that consumable
3. [ ] Mark one lot as sold (qty 1)
4. [ ] Verify consumables auto-apply
5. [ ] Verify profit calculation shows correctly
6. [ ] Submit sale
7. [ ] Go to Sales & Profit and verify sale appears
8. [ ] View order details and verify breakdown is correct

---

## Known Issues to Watch For

- [ ] Migration may need to be run manually if auto-migration fails
- [ ] If columns don't exist, API will gracefully handle it but features will be limited
- [ ] Consumables with no purchases will show £0.00 cost (expected behavior)
